...
#
# 2026-08 논리 기반 재배분 — 데이터 기반 최적화는 정답지(진짜 바닥 라벨)와 검정력
# 부재로 불가능 확인, 대신 이번 세션에서 실측 확인된 신호별 신뢰도를 근거로 재배분:
# - accumulation(25→38): 유일하게 "누가 실제로 사고 있다"는 직접적 수급 행동 증거.
#   percentile 전환·raw하한 게이트·8일 경계 재설계 3차례 수술을 거쳐 가장 정제됨.
# - relative_strength(13→8): "이 시장에서 덜 빠졌다"는 논리 자체가 틀렸을 수 있음 —
#   하락방어 전환 실측(2026-08)에서 오히려 역방향이 악화(손절그룹이 익절그룹보다
#   상대강도가 높음, +9.4%p→+17.6%p) — 신뢰도를 낮춰야 함.
# - dividend_yield(5→3): pbr_low와 개념 중복(둘 다 "역사적으로 싸다") + 무배당
#   종목은 애초에 None이라 절반 가까이 신호 자체가 없음 — 가장 간접적.
# - volume_dryness(18→13): "거래 마름"은 매도 소진(좋음)과 무관심(양면적) 둘 다일
#   수 있어 해석이 갈림 — 과한 비중을 축소.
# - pbr_low(15→17): 역사적 저평가는 비교적 직접적인 가치 증거라 소폭 상향.
# - short_covering(13→10): 위 신호들 대비 상대적으로 하향.
# - volatility_squeeze(11): 유지.
BOTTOM_WEIGHTS = {
    "accumulation": 38,        # 기관·외국인 매집 — 유일한 직접적 수급 행동 증거, 3차 정제됨
    "pbr_low": 17,             # PBR 역사적 저점
    "volume_dryness": 13,      # 거래량 고갈 — 양면적 해석이라 비중 축소
    "short_covering": 10,      # 공매도 감소
    "relative_strength": 8,    # 상대강도 — 하락방어 전환 실측에서 역방향 악화 확인, 신뢰도 하향
    "volatility_squeeze": 11,  # 변동성 수축
    "dividend_yield": 3,       # 배당수익률(조건부) — PBR과 개념 중복, 가장 간접적
}
