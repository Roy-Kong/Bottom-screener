...
다른 로직(매일 스크리닝 + 최대 10종목 동시 보유 + +5% 익절 또는 30일 만기청산,
손절 없음)이라 별도 스크립트로 만들었다 — strategy_backtest_2022.py/
screener.py/signals.py/db_reader.py는 이 스크립트가 함수만 가져다 쓰고
전혀 수정하지 않는다.
...
- 보유 종목: 매수 후 최대 MAX_HOLD_CALENDAR_DAYS(달력 기준 30일, 휴장이면
  다음 거래일로 보정)까지 보유, **손절 없음**(STOP_LOSS_ENABLED=False, 사용자
  지시). 그 안에 그날 고가가 매수가 대비 +5% 이상이면 익절(정확히 목표가에
  체결). 30일 안에 못 찍으면 그 시점(만기일) 종가로 강제청산("만기청산(30일)").
  — strategy_backtest_2022.py가 원래 검증한 방식(30영업일 내 아무 때나
  +X% 터치했는지만 관찰, 도중 하락은 안 봄)에 가장 가까운 형태. 손절 폭을
  -5%→-10%로 늘렸다가 결과가 더 나빠져서(손익비 구조상 승률 66% 이상
  필요한데 실제 49%뿐이라 기대값이 마이너스), 손절 메커니즘 자체가 성과를
  얼마나 깎아먹는지 분리해서 보려고 아예 뺀 버전.
- 매수 시에만 BUY_FEE_PCT(0.33%) 수수료 적용(매도 수수료 없음, 사용자 지시)
  — 매수금액(슬롯 10%)과 별도로 현금에서 추가 차감.
- 빈 슬롯이 생긴 날은 그날 기준 65점 이상(바닥 신호 7개, 기존 가중치)이면서
  동시에 턴어라운드 confirmed(가격계열+수급계열 각 1개 이상 50점 이상) 상태인
  종목만 후보로 삼는다 — 미보유·쿨다운 아닌 종목을 점수순으로 슬롯 수만큼
  뽑아 다음 거래일 시가에 매수. 부족하면 남은 슬롯은 이월(다음 거래일에
  다시 그날 기준으로 재스캔). (REQUIRE_CONFIRMED_TURNAROUND=True, 사용자
  지시로 하드 필터화 — screener.py/strategy_backtest_2022.py는 이 상태를
  정렬 우선순위로만 쓰고 선정 자체엔 안 쓰지만, 이 스크립트는 매일 재진입하는
  구조라 "바닥권이지만 아직 안 돈" 종목까지 사면 하락 추세에서 낙폭과대주를
  계속 사들이다 손절만 반복하는 문제가 실측으로 확인됨 — 2022-09~10월 승률
  21~29% vs 11월 74%, 시장 국면 필터 부재 문제를 턴어라운드 confirmed
  요구로 완화).
- 한 번 매도한 종목은 매도일로부터 30영업일 동안 재매수 후보에서 제외.
- 65점 계산은 이미 검증된 로직(signals.BOTTOM_WEIGHTS 7개 신호, 생존게이트
  OHLC=0/거래정지 필터) 그대로 재사용.
...
STOP_LOSS_PCT = -0.10           # STOP_LOSS_ENABLED=False면 안 씀
STOP_LOSS_ENABLED = False       # 손절 자체를 없앰(사용자 지시) — +5% 익절 또는 30일 만기청산만 존재.
                                 # strategy_backtest_2022.py가 원래 검증한 방식(30영업일 내 아무 때나
                                 # +X% 터치했는지만 관찰, 손절 없음)에 가장 가까운 형태로 되돌려서,
                                 # 손절 메커니즘 자체가 성과를 얼마나 깎아먹는지 분리해서 보기 위함.
MAX_HOLD_CALENDAR_DAYS = 30     # 매수 후 이 기간(달력일) 안에 +5% 미도달이면 만기청산
BUY_FEE_PCT = 0.0033            # 매수 시에만 적용(매도 수수료 없음, 사용자 지시)
COOLDOWN_TRADING_DAYS = 30
REQUIRE_CONFIRMED_TURNAROUND = True  # 바닥65점+턴어라운드confirmed 둘 다여야 매수 후보 (사용자 지시)
EXCLUDED_BOTTOM_SIGNALS = {"pbr_low", "dividend_yield"}  # 바닥점수 조합에서 제외 (사용자 지시)
...
def hold_deadline(buy_date: str, trading_days: list[str]) -> str:
    """매수일로부터 달력 기준 MAX_HOLD_CALENDAR_DAYS일 되는 날. 그날이 휴장이면
       다음 거래일로 보정한다(사용자 지시). 시뮬레이션 기간을 넘어가면 마지막
       거래일(사실상 "기간 끝까지 보유")을 반환한다."""
    buy_dt = dt.datetime.strptime(buy_date, "%Y%m%d").date()
    target_str = scr.yyyymmdd(buy_dt + dt.timedelta(days=MAX_HOLD_CALENDAR_DAYS))
...
def _compute_raw_scores_for_day(day: str, universe: dict, sector_map: dict, sector_names: dict,
    ...
    accum_t9_t48 = dbr.load_accumulation_from_db(dbr.date_range_inclusive(db_dates_sorted, accum_bottom_from, accum_bottom_to))
    accum_recent8 = dbr.load_accumulation_from_db(dbr.date_range_inclusive(db_dates_sorted, accum_recent8_from, latest_date))
...
def combine_scores(raw_entries: list[dict], score_threshold: float = SCORE_THRESHOLD) -> list[dict]:
    """캐시됐거나 방금 계산한 raw 점수 리스트를 현재 가중치(signals.BOTTOM_WEIGHTS,
       EXCLUDED_BOTTOM_SIGNALS 제외)로 조합해 컷라인 통과 종목만 점수 내림차순으로
       반환한다. DB 접근이 전혀 없는 순수 계산이라, 가중치·컷라인만 바꿔 재실험할
       때는 이 함수만 다시 부르면 된다(score_day 전체를 다시 안 돌려도 됨).

       EXCLUDED_BOTTOM_SIGNALS에 든 신호는 weights dict에서 아예 빼버린다 —
       composite_score()가 None인 신호를 자동으로 분모·분자에서 빼고 나머지
       비율로 재정규화하는 것과 같은 방식으로, 이 dict에서 빠진 키는
       weights.get(k, 0)이 0이 되어 자동으로 제외·재정규화된다(pbr_low/
       dividend_yield 값 자체는 raw_entries에 여전히 남아있어 나중에 다시
       켜고 싶으면 캐시 재계산 없이 바로 가능)."""
    out = []
    for e in raw_entries:
        bottom_weights = {k: v for k, v in sg.BOTTOM_WEIGHTS.items() if k not in EXCLUDED_BOTTOM_SIGNALS}
        if e["pbr_caution_sector"] and "pbr_low" in bottom_weights:
        ...
        # 바닥 65점 통과만으로는 매수 안 함 — 턴어라운드 5개 신호(가격계열+수급계열
        # 각 1개 이상 50점 이상)까지 confirmed여야 후보로 인정한다. screener.py/
        # strategy_backtest_2022.py는 이 상태를 정렬 우선순위로만 쓰지만(선정 자체엔
        # 영향 없음), 여기서는 사용자 지시로 하드 필터로 바꿨다 — "바닥권이지만
        # 아직 안 돈" 종목을 걸러서 하락 추세 중 낙폭과대주를 계속 사들이다 손절
        # 반복하는 걸 막기 위함(2022-09~10월 구간에서 실측 확인된 문제).
        if REQUIRE_CONFIRMED_TURNAROUND and status != "confirmed_turnaround":
            continue
...
def run(start: str, end: str, out_csv: str) -> None:
    ...
    for i, day in enumerate(trading_days):
        ...
        if i > 0 and pending_buys:
            ...
            for cand in pending_buys:
                ...
                holdings[tkr] = {"buy_date": day, "buy_price": open_price, "shares": shares,
                                  ...
                                  "deadline": hold_deadline(day, trading_days)}
            pending_buys = []
        ...
        for tkr in sorted(list(holdings.keys())):
            ...
            hit_sl = STOP_LOSS_ENABLED and low <= sl_price
            hit_deadline = day >= pos["deadline"]
            if not (hit_tp or hit_sl or hit_deadline):
                continue
            if hit_sl:
                sell_price, reason = sl_price, f"{STOP_LOSS_PCT*100:.0f}%"
            elif hit_tp:
                sell_price, reason = tp_price, "+5%"
            else:
                sell_price, reason = close, "만기청산(30일)"
...
def print_summary(trade_log: list[dict], equity_curve: list[tuple[str, float]],
    ...
    tp = [t for t in closed if t["sell_reason"] == "+5%"]
    sl = [t for t in closed if t["sell_reason"] == "-10%"]
    expiry = [t for t in closed if t["sell_reason"] not in ("+5%", "-10%")]  # 만기청산(30일)
    wins = tp + [t for t in expiry if t["return_pct"] > 0]  # 만기청산도 수익률>0이면 승리로 집계
    ...
    print(f"  청산 매매: {len(closed)}건 (+5% 익절 {len(tp)}건, -10% 손절 {len(sl)}건, "
          f"만기청산(30일) {len(expiry)}건[중 수익 {sum(1 for t in expiry if t['return_pct']>0)}건])")
    if closed:
        win_rate = len(wins) / len(closed) * 100
        print(f"  승률(수익률>0% 기준, 만기청산 포함): {win_rate:.1f}%")
