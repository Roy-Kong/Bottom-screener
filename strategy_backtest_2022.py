...
def screen_and_score(anchor_date: dt.date, asof: str, use_cache: bool = True) -> list[dict]:
    ...
    if snapshot is None:
        ...
        # data/index_history.sqlite(index_db.py, backfill_index.py로 채움)에서
        # 읽는다 — 상대강도·상대강도가속 신호가 이 값을 쓴다
        # (resolve_benchmark_series). 업종지수는 backfill_index.py
        # --sector-codes로 실제로 채운 코드만 값이 있고, 안 채운 코드는 빈
        # dict가 반환돼 그 종목은 자동으로 시장지수 폴백으로 넘어간다.
    ...
    for tkr, name in universe.items():
        ...
        # accumulation 분모: 거래대금 근사치 -> 실제 시총(cur_market_cap, 이미 생존게이트에서
        # >=MIN_MARKET_CAP으로 확인됨). screener.py와 동일 교체 — 라이브/백테스트 정합성 유지.
