...
def _ensure_lfs_content(path) -> bool:
    """그 날짜 파일이 아직 LFS 포인터 스텁이면(내용을 못 받아온 상태) 그 파일
       하나만 다시 pull 시도한다 — "Pull existing DB files" 스텝이 260개 중
       특정 한두 개를 놓치는 일이 CI에서 실제로 반복 관찰됐다(로컬 재현은 안
       됨 — 원인 불명의 CI 쪽 산발적 누락으로 추정). 매 날짜마다 무조건 pull을
       또 하면 낭비라, 실제로 sqlite3.DatabaseError("file is not a database")
       가 난 경우에만 이 복구를 시도한다. 반환값: 정상 SQLite 파일이 됐으면 True."""
    try:
        conn = sqlite3.connect(str(path))
        conn.execute("SELECT 1 FROM daily_prices LIMIT 1")
        conn.close()
        return True
    ...
    try:
        subprocess.run(
            ["git", "lfs", "pull", "--include", str(path).replace("\\", "/")],
...
def backfill_one_day(ds: str) -> int:
    ...
    if not path.exists():
        return 0
    if not _ensure_lfs_content(path):
        raise RuntimeError(f"{ds}: LFS 콘텐츠 복구 실패(git lfs pull 재시도 후에도 포인터 스텁)")
